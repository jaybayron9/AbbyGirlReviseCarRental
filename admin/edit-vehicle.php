...
if (strlen($_SESSION['alogin']) == 0) {
	...
	if (isset($_POST['submit'])) {
		...
		$sql = $dbh->query("update tblvehicles set VehiclesTitle='{$vehicletitle}',VehiclesBrand='{$brand}',VehiclesOverview='{$vehicleoverview}',PricePerDay='{$priceperday}',FuelType='{$fueltype}',ModelYear='{$modelyear}',SeatingCapacity='{$seatingcapacity}',AirConditioner='{$airconditioner}',PowerDoorLocks='{$powerdoorlocks}',AntiLockBrakingSystem='{$antilockbrakingsys}',BrakeAssist='{$brakeassist}',PowerSteering='{$powersteering}',DriverAirbag='{$driverairbag}',PassengerAirbag='{$passengerairbag}',PowerWindows='{$powerwindow}',CDPlayer='{$cdplayer}',CentralLocking='{$centrallocking}',CrashSensor='{$crashcensor}',LeatherSeats='{$leatherseats}' where id='{$id}' ");
								...
								<div class="col-md-12">
									<div class="panel panel-default">
										...
										<div class="panel-body">
											...
											<?php
											$id = intval($_GET['id']);
											$sql = $dbh->query("SELECT tblvehicles.*,tblbrands.BrandName,tblbrands.id as bid from tblvehicles join tblbrands on tblbrands.id=tblvehicles.VehiclesBrand where tblvehicles.id='{$id}'");
											$cnt = 1;
											if ($sql->num_rows > 0) {
