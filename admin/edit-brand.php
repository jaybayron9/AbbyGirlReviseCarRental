...
$brand=$_POST['brand'];
$id=$_GET['id'];
$sql= $dbh->query("update  tblbrands set BrandName='{$brand}' where id='{$id}'");
$lastInsertId = $dbh->query("SELECT LAST_INSERT_ID() from tblbrands");
...
<?php	
$id=$_GET['id'];
$ret= $dbh->query("select * from tblbrands where id='{$id}'");
$cnt=1;
if($ret->num_rows > 0)
{
